export class FormController {
    ...
    async handleSubmit() {
        ...
        this.view.setOutput('Preparando os recursos de IA...');

        try {
            // Translator, LanguageDetector e LanguageModel podem precisar baixar
            // modelos. Iniciamos os três antes do primeiro await para preservar o
            // gesto do clique que enviou o formulário.
            const translationInitialization = this.translationService.initialize(
                (message) => this.view.setOutput(message)
            );

            const aiSessionCreation = this.aiService.createSession(
                question,
                temperature,
                topK,
                file,
            ...
            const [, aiResponseChunks] = await Promise.all([
